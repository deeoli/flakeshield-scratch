...
def find_flakeshield_comment(github_token: str, repo_owner: str, repo_name: str, issue_number: int) -> int | None:
...
def update_or_create_comment(
    ...
    """
    existing_id = find_flakeshield_comment(github_token, repo_owner, repo_name, issue_number)
    ...
    if existing_id:
        ...
        response = requests.patch(url, json={"body": markdown}, headers=headers, timeout=10)
        response.raise_for_status()
        return existing_id
    else:
        # Create new comment
        url = f"https://api.github.com/repos/{repo_owner}/{repo_name}/issues/{issue_number}/comments"
        response = requests.post(url, json={"body": markdown}, headers=headers, timeout=10)
        response.raise_for_status()
